 ...
 *
 * `make frontend-dev` assumes a local API on :8080. This config keeps the
 * local frontend — HMR and all — and points only the data at prod, so a CSS or
 * markup change can be looked at against the real corpus.
 *
 ...
 *      login. For those, use the local authenticated rig instead:
 *      `make dev-auth-up` then `make frontend-dev-auth` (:3200), which serves
 *      this branch's API against a throwaway database where writes are safe.
