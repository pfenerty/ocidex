...
- **Authorization change (a new capability, role, or auth class) → read ADR-046 first.** It is
  the model in force and supersedes ADR-025. The capability matrix is compile-time in
  `internal/authz`; `docs/AUTH_MATRIX.md` is generated by `make auth-matrix` and enforced in CI.
- **Identity change (a new provider, the login/callback flow, or `user_identity`) → read
  ADR-047 first.** Providers live behind `internal/auth.Provider`; nothing above that
  package may name an issuer. An identity matches on `(provider, subject)` and never on
  email.
...
| Auth/authz matrix (generated) | [docs/AUTH_MATRIX.md](docs/AUTH_MATRIX.md) |
| Namespace membership, roles, capabilities, key ceilings | [ADR-046](docs/adr/0046-namespace-membership-rbac.md) |
| Identity providers, OIDC, account linking | [ADR-047](docs/adr/0047-provider-agnostic-identity.md) |
