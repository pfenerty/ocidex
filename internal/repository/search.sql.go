...
WITH scope AS (
    ...
    FROM component c1 WHERE c1.sbom_id = $1 AND c1.purl IS NOT NULL
    UNION
    SELECT DISTINCT c2.purl::text AS purl, c2.source_purl::text AS match_purl
    FROM component c2 WHERE c2.sbom_id = $1 AND c2.purl IS NOT NULL AND c2.source_purl IS NOT NULL
), findings AS (
...
WHERE c.sbom_id = $1
  ...
  AND (
    NOT $2::boolean
    OR (c.name, COALESCE(c.group_name, ''), c.id) > ($3::text, $4::text, $5::uuid)
  )
ORDER BY
    CASE WHEN $6::boolean THEN COALESCE(sev.critical, 0) * $7::int END DESC,
    CASE WHEN $6::boolean THEN COALESCE(sev.high,     0) * $7::int END DESC,
    CASE WHEN $6::boolean THEN COALESCE(sev.medium,   0) * $7::int END DESC,
    CASE WHEN $6::boolean THEN COALESCE(sev.low,      0) * $7::int END DESC,
    CASE WHEN $6::boolean THEN COALESCE(sev.unknown,  0) * $7::int END DESC,
