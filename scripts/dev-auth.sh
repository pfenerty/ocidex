...
# Mints the five personas, each with a read-write and a read-only API key. Keys
...
"
        for scope in read-write read; do
            key="$(mint_key)"
            hash="$(printf '%s' "$key" | shasum -a 256 | cut -d' ' -f1)"
            prefix="${key:0:8}"
            sql+="
INSERT INTO api_key (user_id, name, key_hash, prefix, scope)
SELECT id, '$user $scope', '$hash', '$prefix', '$scope'
FROM ocidex_user WHERE github_id = $gid;
"
            var="OCIDEX_DEV_KEY_$(printf '%s' "$user" | tr '[:lower:]' '[:upper:]')"
            [ "$scope" = "read-write" ] && var="${var}_RW" || var="${var}_RO"
            env_body+="$var=$key
"
            # The vite dev config and scripts/dev-fixtures.py both predate the
            # persona roster; devadmin's read-write key keeps the old name so
            # neither has to know about personas to keep working.
            [ "$user" = "devadmin" ] && [ "$scope" = "read-write" ] && \
...
# One read-write (_RW) and one read-only (_RO) key per persona; see PERSONAS.
$env_body
DATABASE_URL=$DB_URL
ENV
...
case "${1:-up}" in
    ...
    status)
        ...
        ;;
    # `key` alone is devadmin's read-write key (what the vite config injects);
