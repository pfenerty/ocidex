...
func TestMainModuleVersionBackfillMigration(t *testing.T) {
	...
	// setupTestDB already ran migration 00026; replay its DO block so the test
	// simulates "rows existed before the migration ran" semantics. The body
	// mirrors db/migrations/00026_backfill_unknown_main_module_version.sql —
	// drive from a small main_module CTE, use idx_component_sbom_id, match by
	// LIKE not regex (see ocidex-3d4 for why the original was too slow).
...
BEGIN
    ...
    FOR rec IN SELECT sbom_id, subject_version, module_path FROM _main_module LOOP
        UPDATE component c
        SET version = rec.subject_version
